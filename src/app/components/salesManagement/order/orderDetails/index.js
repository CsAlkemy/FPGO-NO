    ...
    <div className="create-product-container">
        ...
        <div className="rounded-sm bg-white p-0 md:px-20">
          <div className=" header-click-to-action">
            <div className="flex items-center justify-start w-full sm:w-auto px-16 sm:px-0">
              <div className="header-text header6 ">{t("label:orderDetails")}</div>
              <span className={`${info.status.toLowerCase() === "paid" ? "bg-confirmed" : (info.status.toLowerCase() === "sent" ? "bg-pending" : "bg-rejected")} rounded-4 px-16 py-4 body3 ml-10`}>{info.status.charAt(0).toUpperCase() + info.status.slice(1).toLowerCase()}</span>
            </div>
            <Hidden smDown>
              <div className="button-container-product">
                ...
                {
                  (info.status.toLowerCase() === "sent" || info.status.toLowerCase() === "paid") &&
                  <Button
                    color="secondary"
                    variant="contained"
                    ...
                    disabled={user.role[0] === FP_ADMIN}
                    startIcon={info.status.toLowerCase() === "paid" ? <UTurnLeftIcon className="rotate-90" /> : <RedoOutlined />}
                    onClick={() => handleResendRefundOrder()}
                  >
                    {info.status.toLowerCase() === "paid" ? t("label:refuncOrder") : t("label:resendOrder")}
                  </Button>
                }
              </div>
            </Hidden>
      ...
      <Hidden smUp>
        <div className="fixed bottom-0 grid grid-cols-1 justify-center items-center gap-10 w-full mb-10 mt-10 px-20">
          ...
          {
            (info.status.toLowerCase() === "sent" || info.status.toLowerCase() === "paid" || info.status.toLowerCase() === "invoiced") &&
            <Button
              color="secondary"
              variant="contained"
              ...
              //disabled = {isValid ? true: false }
              disabled={user.role[0] === FP_ADMIN}
              startIcon={info.status.toLowerCase() === "paid" || info.status.toLowerCase() === "invoiced" ? <UTurnLeftIcon className="rotate-90" /> : <RedoOutlined />}
              onClick={() => handleResendRefundOrder()}
            >
              {info.status.toLowerCase() === "paid" || info.status.toLowerCase() === "invoiced" ? t("label:refundOrder") : t("label:resendOrder")}
            </Button>
          }
        </div>
      </Hidden>
      <OrderModal
        ...
        customerPhone={info.customerDetails.countryCode && info.customerDetails.msisdn ? info.customerDetails.countryCode + info.customerDetails.msisdn : ""}
