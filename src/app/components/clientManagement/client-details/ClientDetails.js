...
const ClientDetails = () => {
  ...
  useEffect(() => {
    ...
    defaultValue.costLimitforOrder = info?.apticInformation?.costLimitForOrder
      ...
      : "";
    defaultValue.invoicewithRegress = info?.apticInformation?.invoiceWithRegress
    ...
    defaultValue.APTIEngineCuserName = info?.apticInformation
      ...
      : "";
    defaultValue.APTIEnginePassword = info?.apticInformation?.backOfficePassword
    ...
    if (info?.settings?.vatRates && info?.settings?.vatRates.length) {
      for (let i = 0; i < info?.settings?.vatRates.length; i++) {
        ...
        setValue(`vat[${i}].vatValue`, info?.settings?.vatRates[`${i}`].value);
        setValue(
