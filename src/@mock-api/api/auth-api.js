...
mock.onGet('/api/contexts/login').reply(async (config) => {
...
mock.onGet('/api/contexts/access-token').reply((config) => {
  const data = JSON.parse(config.data);
...
mock.onPost('/api/contexts/sign-up').reply((request) => {
...
mock.onPost('/api/contexts/user/update').reply((config) => {
  const data = JSON.parse(config.data);
  const { user } = data;
