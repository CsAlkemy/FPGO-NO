...
class FuseUtils {
  ...
  static hasPermission(authArr, userRole) {
    /**
     * If contexts array is not defined
     * Pass and allow
     */
    if (authArr === null || authArr === undefined) {
      // console.info("contexts is null || undefined:", authArr);
      return true;
    }
    if (authArr.length === 0) {
      /**
       * if contexts array is empty means,
       * allow only user role is guest (null or empty[])
       */
      // console.info("contexts is empty[]:", authArr);
      return !userRole || userRole.length === 0;
    }
    /**
     * Check if user has grants
     */
    // console.info("contexts arr:", authArr);
    /*
